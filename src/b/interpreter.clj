;; # The Approach 
;; We use a kind of compilation approach to calculate expressions and
;; predicates. The AST generated by the B parser is turned into a
;; Clojure datastructure, e.g., #EXPRESSION 4+2 will result in the
;; datastructure (b.interpreter/AAddExpression
;; (b.interpreter/AIntegerExpression "4")   (b.interpreter/AIntegerExpression "1"))  
...
;; We use the maybe-monad-transformer on the state monad. The result
;; is used in the animator. We cannot use the state moand itself
;; because we need to deal with undefined results (e.g., if we divide
;; by zero.)
(def wd-state-m (maybe-t state-m))

;; We want to promote regular clojure functions to monadic
;; versions. In the same step, we want to rename them to reflect the
;; names of the 
(defmacro lift
  ([name op n]
     `(def ~name (with-monad wd-state-m (m-lift ~n ~op))))
